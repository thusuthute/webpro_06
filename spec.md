...
stateDiagram-v2
    [*] --> パンの一覧表示: 
    パンの一覧表示 --> 詳細表示: 商品名をクリック
    詳細表示 --> 編集画面: 編集ボタンをクリック
    編集画面 --> 登録: 
    登録 --> 一覧表示
   パンの一覧表示 --> 追加画面: 追加ボタンをクリック
    追加画面 --> 登録: 
    パンの一覧表示 --> 削除画面: 削除ボタンをクリック
    削除画面 --> 一覧表示
```
...
stateDiagram-v2
    [*] --> 月単位での一覧表示: 
    月単位での一覧表示 --> 日にち単位での詳細表示: 対応する月をクリック
    日にち単位での詳細表示 --> 詳細の編集画面: 編集ボタンをクリック
    詳細の編集画面 --> 登録: 
    登録 --> 一覧表示
    詳細の追加画面 --> 登録: 
    詳細の削除画面 --> 一覧表示
    月単位での一覧表示 --> 月の追加画面: 追加ボタンをクリック
    月の追加画面 --> 登録
    日にち単位での詳細表示 -->詳細の追加画面: 追加ボタンをクリック
    日にち単位での詳細表示 -->詳細の削除画面: 削除ボタンをクリック
    日にち単位での詳細表示 -->月単位での一覧表示: 戻るボタンをクリック
